...
Target::Target(const string& name) : kId_(Target::idcounter++), kName_(name), inord_(0), command_(""), is_target_(true) {
}

Target::~Target() {
	BOOST_FOREACH(Target* t, dependent_targets_)
		t->dependencies_.remove(this);
	BOOST_FOREACH(Target* t, dependencies_)
		t->dependent_targets_.remove(this);
}
...
void DependencyGraph::RemoveNotMarkedTargets() {
	/* delete those which should not */
	std::vector<Target*> new_all_targets;
	BOOST_FOREACH(Target* t, all_targets_)
		if (t->is_target_)
			new_all_targets.push_back(t);
		else
			delete t;
	all_targets_ = new_all_targets;

	/* sort, update main_targets_ and leaf_targets_,
	 * shoud not be circular if it was not before
	 */
	ReinitInord();
	TopologicalSort();
}

void DependencyGraph::TrimToTargets(std::vector<std::string> targets) {
	if (targets.empty()) return;
	sort(targets.begin(), targets.end());
	
	BOOST_FOREACH(Target* t, all_targets_)
		t->MarkSubtreeIfTarget(false);
	BOOST_FOREACH(Target* t, all_targets_)
		if (binary_search(targets.begin(), targets.end(), t->kName_))
			t->MarkSubtreeIfTarget(true);

	RemoveNotMarkedTargets();
}

void DependencyGraph::DeleteBlah() {
	BOOST_FOREACH(Target* t, all_targets_)
		if (t->kName_ == "blah")
			t->MarkSubtreeIfTarget(false);
	RemoveNotMarkedTargets();
